...
/* ═══════════════════════════════════════════
   Main Builder View
   ═══════════════════════════════════════════ */
export interface AgentsBuilderViewProps {
  onBack: () => void;
  agentTitle?: string;
  initialNodes?: FlowNode[];
  agentConditions?: ConditionBlock[];
}
...
export function AgentsBuilderView({
  ...
  const [nodes, setNodes] = useState(initialNodes ?? defaultFlowNodes);
  const [selectedNodeId, setSelectedNodeId] = useState<string | null>((initialNodes ?? defaultFlowNodes)[0]?.id ?? null);
  const [conditions, setConditions] = useState(agentConditions ?? initialConditions);
        ...
        <MainCanvasViewHeader
          title={
            <span className="flex min-w-0 items-center gap-3">
              ...
              <span className="truncate">{agentTitle ?? "Review response agent replying autonomously"}</span>
            </span>
          }
          actions={
            <div className="flex items-center gap-2">
