...
export function CustomizeShareDrawer({ open, onClose, context }: CustomizeShareDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-[420px] h-full bg-white dark:bg-background border-l border-gray-50 dark:border-border flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-light-grayish-blue dark:border-border shrink-0">
          <div className="flex items-center gap-2">
            <Palette className="w-4 h-4 text-brand-color" />
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          <div>
            <p className="text-[12px] text-gray-90 dark:text-muted-foreground uppercase tracking-[0.5px] mb-3" style={{ fontWeight: 400 }}>Presentation</p>

            {/* Title */}
            <div className="mb-3">
              <label className="block text-[12px] text-gray-80 dark:text-muted-foreground mb-1.5 tracking-[-0.24px]" style={{ fontWeight: 400 }}>Title</label>
              <input
                type="text"
                value={titleOverride}
            ...
            {/* Subtitle */}
            <div className="mb-3">
              <label className="block text-[12px] text-gray-80 dark:text-muted-foreground mb-1.5 tracking-[-0.24px]" style={{ fontWeight: 400 }}>Subtitle</label>
              <input
                type="text"
            ...
            {/* Layout */}
            <div className="mb-3">
              <label className="block text-[12px] text-gray-80 dark:text-muted-foreground mb-1.5 tracking-[-0.24px]" style={{ fontWeight: 400 }}>Layout</label>
            ...
            {/* Theme */}
            <div className="mb-3">
              <label className="block text-[12px] text-gray-80 dark:text-muted-foreground mb-1.5 tracking-[-0.24px]" style={{ fontWeight: 400 }}>Theme</label>
              <div className="flex gap-2">
                {themes.map((t) => (
                  <button
                    key={t.id}
                    ...
                    style={{ fontWeight: 400 }}
                  >
                    <span className="w-3 h-3 rounded-full border border-new-selected-color dark:border-border" style={{ background: t.color }} />
                    {t.label}
                  </button>
                ))}
              </div>
            ...
            {context.supportsBranding && (
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded accent-brand-color"
                />
                <span className="text-[13px] text-gray-900 dark:text-foreground" style={{ fontWeight: 400 }}>Include company branding</span>
              </label>
            )}
          </div>

          <div className="h-px bg-light-grayish-blue dark:bg-muted" />

          {/* Delivery section */}
          <div>
            <p className="text-[12px] text-gray-90 dark:text-muted-foreground uppercase tracking-[0.5px] mb-3" style={{ fontWeight: 400 }}>Delivery</p>

            {/* Format */}
            <div className="mb-3">
              <label className="block text-[12px] text-gray-80 dark:text-muted-foreground mb-1.5 tracking-[-0.24px]" style={{ fontWeight: 400 }}>Export format</label>
              <div className="flex gap-2">
