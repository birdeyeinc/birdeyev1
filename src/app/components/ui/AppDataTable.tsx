...
// bg-background stays opaque always — hover/selected tint is layered via after: pseudo-element
// so scrolled columns never bleed through the sticky cell.
const STICKY_LEADER_CELL_BASE =
  "sticky border-r border-transparent bg-background transition-[box-shadow,border-color] duration-200 ease-out overflow-visible" +
  " after:pointer-events-none after:absolute after:inset-0 after:transition-colors" +
  " group-hover/table-row:after:bg-muted/30 group-data-[state=selected]/table-row:after:bg-muted/60";
...
export interface AppDataTableProps<TData> {
  ...
  persist?: boolean;
  /** Directory rows: put **`APP_DATA_TABLE_PRIMARY_ROW_LABEL_CLASS`** (`appDataTableCellClasses.ts`) on the main label for token blue on hover/selected. */
  onRowClick?: (row: TData) => void;
  /** Optional floating action rendered on each row; appears on hover/focus and does not trigger row click. */
  rowHoverAction?: (row: TData) => ReactNode;
  ...
  /** When true, omit the built-in Columns control. Pair with `columnSheetOpen` + `onColumnSheetOpenChange` + {@link AppDataTableColumnSettingsTrigger} to move the trigger to a page header, or omit all three to suppress column management entirely. */
...
export function AppDataTable<TData>({
  ...
  // Re-scale column widths proportionally when the container shrinks below current total.
  // Preserves manual resizing at larger widths while preventing horizontal overflow.
  useLayoutEffect(() => {
    if (sizingEmpty || layoutW < 1 || visibleOrderedIds.length === 0) return;
    const currentTotal = table.getTotalSize();
    if (currentTotal <= layoutW) return;
    const ratio = layoutW / currentTotal;
    setColumnSizing((prev) => {
      const next: Record<string, number> = {};
      for (const id of visibleOrderedIds) {
        const prev_size = (prev as Record<string, number>)[id] ?? 150;
        next[id] = Math.max(40, Math.round(prev_size * ratio));
      }
      return next;
    });
  }, [layoutW, sizingEmpty, visibleOrderedIds, table, setColumnSizing]);
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="border-b border-border hover:bg-transparent">
                ...
                {rowHoverAction ? (
                  <TableHead
                    className={cn(
                      "h-auto w-12 min-w-[3rem] sticky right-0 z-[26] bg-background",
                      densityHead,
                    )}
            ...
            {table.getRowModel().rows.map((row) => {
              ...
              return (
                <TableRow
                  ...
                  className={cn(
                    "group/table-row relative border-b border-border hover:bg-muted/30 data-[state=selected]:bg-muted",
                ...
                {rowHoverAction ? (
                  <TableCell
                    className={cn(
                      "w-12 min-w-[3rem] sticky right-0 z-[21] bg-background group-hover/table-row:bg-muted/30 group-data-[state=selected]/table-row:bg-muted",
                      densityCell,
                    )}
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div className="pointer-events-none flex items-center justify-center opacity-0 transition-opacity duration-150 group-hover/table-row:opacity-100 group-focus-within/table-row:opacity-100">
                      <div className="pointer-events-auto">
                        {rowHoverAction(row.original)}
                      </div>
                    </div>
                  </TableCell>
                ) : null}
