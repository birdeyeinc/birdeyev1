...
export function SettingsSectionCard({ section, sectionRef, onItemClick }: SettingsSectionCardProps) {
  ...
  return (
    ...
    >
      {/* header */}
      <div>
        <h2 className={MAIN_VIEW_PRIMARY_HEADING_CLASS}>{section.label}</h2>
      ...
      {section.banner && !bannerDismissed && (
        <div className="flex items-center gap-2 rounded-md bg-primary/5 px-4 py-2">
          <Info
            size={14}
      ...
      {/* 3-col item grid with consistent vertical breathing room */}
      <div className="grid grid-cols-3 gap-x-6 gap-y-4 items-start">
        {section.items.map((item) => (
          <SettingsItemRow
            ...
            iconAccent={section.accentColor}
            onClick={() => onItemClick?.(section.key, item.key)}
          />
        ))}
      </div>
    </div>
