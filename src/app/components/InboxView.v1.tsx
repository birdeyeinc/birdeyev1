...
import { WhatsAppCallBubble } from "@/app/components/WhatsAppCallBubble";
...
} from "@/app/components/inbox/AppointmentCard";
...
interface ChatMessage {
  ...
  sentVia?: string;
  senderName?: string;
  type?: "appointment-booked" | "appointment-rescheduled" | "waitlist-sms";
...
interface ConversationDetail {
  ...
  dateSeparators?: Record<string, string>;
}
...
const conversations: Conversation[] = [
  ...
  // ── Waitlist conversations ──
  {
    id: "waitlist-sarah",
    name: "Sarah Mitchell",
    preview: "Appointment confirmed for May 14 at 10:00 AM",
    lastMessage: "Appointment confirmed for May 14 at 10:00 AM",
    date: "Today",
    location: "SF · Today",
    team: "Waitlist agent",
    teamIcon: "kelsy",
  ...
  },
  {
    id: "waitlist-liam",
    name: "Liam Walker",
    preview: "You're #1 on the waitlist for a New patient appointment",
    lastMessage: "You're #1 on the waitlist for a New patient appointment",
    date: "Today",
    location: "Boston · Today",
    team: "Waitlist agent",
    teamIcon: "kelsy",
    unread: true,
    category: "appointments",
  },
  {
    id: "waitlist-james",
    name: "James Thornton",
    preview: "A slot has opened — May 9 at 2:00 PM with Dr. Patel",
    lastMessage: "A slot has opened — May 9 at 2:00 PM with Dr. Patel",
    date: "Today",
    location: "Chicago · Today",
    team: "Waitlist agent",
    teamIcon: "kelsy",
    unread: true,
    category: "appointments",
    ...
    messages: [
      {
        ...
        time: "03:27 PM",
        appointment: {
          patientName: "suparbhat thakkar",
          patientInitials: "ST",
          category: "Other",
          appointmentType: "Routine Checkup",
          bookingDatetime: "Fri Apr 24, 2026 • 06:30 PM - 07:00 PM IST",
        },
      },
      {
        id: "m2",
        sender: "agent",
        type: "appointment-rescheduled" as const,
        text: "",
        time: "03:31 PM",
        senderName: "Shashank Srivastav",
        appointment: {
          patientName: "suparbhat thakkar",
          patientInitials: "ST",
          category: "Other",
          appointmentType: "Routine Checkup",
          bookingDatetime: "Sun Apr 26, 2026 • 06:30 PM - 07:00 PM IST",
        },
      },
    ],
  },
  // ── Waitlist conversation details ──
  "waitlist-sarah": {
    contactName: "Sarah Mitchell",
    assignedTo: "Waitlist agent",
    assignedAvatar: "https://i.pravatar.cc/80?img=1",
    dateSeparator: "Thu • May 9",
    dateSeparators: WAITLIST_DATE_BREAKS,
    messages: SARAH_MITCHELL_CONVERSATION as unknown as ChatMessage[],
  },
  "waitlist-liam": {
    contactName: "Liam Walker",
    assignedTo: "Waitlist agent",
    assignedAvatar: "https://i.pravatar.cc/80?img=8",
    dateSeparator: "Fri • May 9",
    messages: [
      {
        id: "m1",
        sender: "agent",
    ...
    messages: [
      ...
      },
      {
        ...
        waitlistSms: {
          ...
          provider: "Dr. Alan Patel",
          location: "Chicago, IL",
          slotDatetime: "May 9, 2026 · 2:00 PM",
        },
      },
    ],
  },
...
function ChatBubble({ msg }: { msg: ChatMessage }) {
  ...
  const isAppointment =
    msg.type === "appointment-booked" || msg.type === "appointment-rescheduled";
  const isWaitlistSms = msg.type === "waitlist-sms" && msg.waitlistSms != null;

  return (
    <div
      className={`flex flex-col ${isAgent ? "items-end" : "items-start"} mb-2`}
    >
      {isWaitlistSms && msg.waitlistSms ? (
        <WaitlistSmsCard data={msg.waitlistSms} />
      ) : isAppointment && msg.appointment ? (
        <AppointmentCard
          variant={msg.type === "appointment-rescheduled" ? "rescheduled" : "booked"}
          data={msg.appointment}
        />
      ) : (
        <div
...
/* ═════════════════════════════════════
   Main Inbox View
   ═════════════════════════════════════ */
export function InboxView({ activeL2Key }: { activeL2Key?: string }) {
  const isAppointmentsFilter = activeL2Key === "Human actions/Appointments";
  ...
  // When the appointments filter activates and the current selection isn't visible,
  // auto-select the first appointment conversation.
  useEffect(() => {
    if (!isAppointmentsFilter) return;
    const appointmentConvs = conversations.filter((c) => c.category === "appointments");
    if (appointmentConvs.length > 0 && !appointmentConvs.some((c) => c.id === selectedId)) {
      selectConversation(appointmentConvs[0].id);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAppointmentsFilter]);

  const visibleConversations = isAppointmentsFilter
    ? conversations.filter((c) => c.category === "appointments")
    : conversations;
        ...
        <div ref={chatMessagesRef} className="min-h-0 flex-1 overflow-y-auto">
          {detailLoading ? (
            ...
            <div className="px-6 py-5">
              ...
              {/* WhatsApp-style call bubble — shown as first message if call recording exists */}
              {CALL_BY_ID[selectedId] && (
                <WhatsAppCallBubble
                  record={CALL_BY_ID[selectedId]}
                  onExpand={() => setCallPlayerOpen(true)}
                />
              )}
...
function InboxCallPlayerSheet({ selectedId, open, onClose }: { selectedId: string; open: boolean; onClose: () => void }) {
  const record = CALL_BY_ID[selectedId];
  if (!record || !open) return null;
  return (
    <Sheet open onOpenChange={onClose}>
      <SheetContent side="right" inset="floating" floatingSize="xl" className={FLOATING_SHEET_FRAME_CONTENT_CLASS}>
        <CallRecordingPlayer record={record} />
      </SheetContent>
    </Sheet>
  );
}