...
function Sparkline({ data }: { data: number[] }) {
  ...
  return (
    <svg width="120" height="20" className="opacity-50">
      <polyline fill="none" stroke="currentColor" strokeWidth="1.5" points={pts} className="text-brand-color" />
    </svg>
  );
}
...
export function AgentActivityView({ onConfigure }: AgentActivityViewProps) {
  ...
  return (
    <div className="flex flex-col min-h-0 overflow-hidden flex-1">
      {/* Agent header */}
      <div className="flex shrink-0 items-center justify-between border-b border-comparison-0-star dark:border-border bg-white dark:bg-background px-6 py-3 transition-colors">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-90 dark:text-muted-foreground">Automatically drafts and sends review replies across all channels</p>
          </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 rounded-lg border border-comparison-0-star dark:border-border bg-gray-10 dark:bg-app-shell-rail p-0.5">
              ...
              <button
                ...
                className={cn(
                  ...
                  autonomy === m
                    ...
                    : "text-gray-200 dark:text-muted-foreground hover:text-gray-900 dark:hover:text-gray-50",
                )}
              >
      ...
      {/* Metrics row */}
      <div className="grid shrink-0 grid-cols-6 divide-x divide-light-grayish-blue dark:divide-border border-b border-comparison-0-star dark:border-border bg-white dark:bg-background transition-colors">
        {METRICS.map((m) => (
          <div key={m.label} className="flex flex-col gap-1 px-5 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-90 dark:text-muted-foreground">{m.label}</p>
            <div className="flex items-end justify-between gap-2">
              <p className="text-[20px] font-semibold tabular-nums text-gray-900 dark:text-foreground">{m.value}</p>
              <Sparkline data={m.spark} />
            </div>
            <p className={cn("text-[11px] font-medium", m.up === true ? "text-emerald-600" : m.up === false ? "text-red-500" : "text-gray-90")}>
      ...
      {/* Activity table */}
      <div className="min-h-0 flex-1 overflow-hidden bg-gray-10 dark:bg-app-shell-gutter transition-colors">
        <AgentActivityTable
          rows={ACTIVITY_ROWS}
