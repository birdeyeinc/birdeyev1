...
export function L2NavLayout({
  ...
  // Auto-expand the section containing the active item when it changes
  useEffect(() => {
    const slashIdx = active.indexOf("/");
    if (slashIdx < 0) return;
    const sectionLabel = active.slice(0, slashIdx);
    setExpanded(prev => prev[sectionLabel] === false ? { ...prev, [sectionLabel]: true } : prev);
  }, [active]);
