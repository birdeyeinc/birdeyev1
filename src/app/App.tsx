...
import { AppointmentWidgetsView } from "./components/AppointmentWidgetsView.v1";
...
import { IntakeFormsView } from "./components/IntakeFormsView.v1";
...
export default function App() {
  ...
  // Stable ref so navTo's guard never goes stale between renders.
  ...
  // ── URL is the single source of truth for ALL navigation ─────────────────
  /** Active L1 view — derived from URL pathname. */
  ...
  /**
   * L2 key scoped to the current view only — prevents URL keys from one module
   * leaking into another module's active state.
   */
  ...
  /**
   * Stable navigate helper — skips if path matches current pathname (via ref,
   * never stale), uses replace so L2 clicks don't bloat browser history.
   */
  const navTo = useCallback((path: string) => {
    if (pathnameRef.current !== path) navigate(path, { replace: true });
  }, [navigate]); // stable — pathnameRef is mutable, not a dep

  // ── Per-module L2 active keys (each isolated to its own view) ────────────
  ...
  // Inbox L2 key: use direct React state so clicks update the filter immediately,
  // regardless of whether the URL route resolves. URL navigation is best-effort
  // (supports bookmarks/refresh); state is the authoritative source for InboxView.
  ...
  useEffect(() => {
    if (currentView !== "contacts") {
      // setContactsL2Active is now a navigate() call — don't call it here or it
      // redirects to /contacts every time the user leaves the contacts view.
      // The URL-based l2ForView already resets to the fallback automatically.
      setContactsSheetMode("none");
      setContactsDetailId(null);
      setContactsQuickViewId(null);
    }
  ...
  return (
    <MonitorNotificationsProvider
      onNavigateToMonitor={() => {
        setMynaChatExpanded(false);
        navigate(viewToDefaultPath("agents-monitor"));
      }}
    >
    <AppEntryWithSplash>
          ...
          >
            ...
            {currentView === "business-overview" ? (
              ...
              appointmentsL2ShowsCalendarCanvas(appointmentsL2Active) ? (
                ...
                /* Single stable instance — avoids unmount/remount jerk when switching */
                <WaitlistView
