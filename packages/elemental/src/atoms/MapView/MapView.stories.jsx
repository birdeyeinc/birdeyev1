...
const getLocationList = (localSeoData) => {
    return localSeoData?.["locationsList"];
}

const getMarkerValue = (locationItem) => {
    return locationItem?.["localSeoRank"]?.["localSeoRank"].toFixed(1);
}

const getLocationId = (locationItem) => {
    return locationItem?.["id"];
}


const getMarkerIconcolor = (currentMarkerValue) => {
    let iconColor;
    if (currentMarkerValue >= 0 && currentMarkerValue <= 5) {
      iconColor = "#4CAE3D";
      } else if (currentMarkerValue >= 5 && currentMarkerValue < 10) {
          iconColor = "#FBC123";
      } else {
          iconColor = "#DE1B0C";
      }
...
const getMarkerIcon = (marker) => {
    const currentMarkerValue = marker.currentLocationValue;
    const width = 38;
    const height = 26;
    const fontSize = 14;
    const padding = 6;
    const borderRadius = 5;
    let iconColor = getMarkerIconcolor(currentMarkerValue);
    const opacity = 1;
    ...
    drawRoundedRect(context, padding, padding, width, height, borderRadius);
    context.font = `${fontSize}px Inter`;
    context.fillStyle = "#ffffff";
    context.textAlign = "center";
    context.textBaseline = "middle";
    context.fillText(currentMarkerValue, canvas.width / 2, canvas.height / 2);
    return canvas.toDataURL();
};


const getMapTooltipJsx = (marker) => {
    const { name } = marker;
    return (
...
const getClusterViewTooltipJsx = (markers) => {
  return (
    <div>{markers?.length} locations</div>
  );
}
...
Default.args = {
    mapData: {
      ...
      "localSeoScore": 92.3,
      "localSeoScoreDelta": 83.3,
      "highRankingLocations": 5,
      "mediumRankingLocations": 2,
      "lowRankingLocations": 0,
      "locationsList": [
  {
    "id": 1430515,
  ...
  },
  {
    ...
    getMarkerList: getLocationList,
    getMarkerId: getLocationId,
    getMarkerValue: getMarkerValue,
    getMarkerIcon: getMarkerIcon,
    getMarkerIconcolor: getMarkerIconcolor,
