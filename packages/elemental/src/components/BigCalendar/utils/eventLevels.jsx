...
export function stableSortEvents(events, accessors) {
    return events
        .map((event, index) => ({ ...event, _originalIndex: index }))
        .sort((a, b) => {
            const aStart = new Date(accessors.start(a));
            const bStart = new Date(accessors.start(b));
            const diff = aStart - bStart;
            return diff === 0 ? a._originalIndex - b._originalIndex : diff;
        })
