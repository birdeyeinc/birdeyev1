...
import { resetForSafari } from "./WeekDnDFunctions";
// import { compareWithBusinessType } from "utils";

export default class TimeGrid extends Component {
    constructor(props) {
        super(props);

        this.state = { gutterWidth: undefined, isOverflowing: null , selectedEvent: {}, currentDate: moment(), rotatedNode: null, currentColumnInfo: { ele: null, id: null, prevDay:null } };
        this.isReseller = false; //compareWithBusinessType(window?.BE?.business, window?.BE?.business?.type, {reseller: true});
        this.scrollRef = React.createRef();
        this.contentRef = React.createRef();
        this._scrollRatio = null;
        this.noOfPostInDay = {};
        this.noOfPostsPerDay = !props.isSocial ? Infinity : 10;
    }
  ...
  renderEvents(range, events, backgroundEvents, now) {
      ...
      return resources.map(([id, resource], i) =>
          range.map((date, jj) => {
              ...
              if (!this.props.calendarSimmerStatus && this.noOfPostsPerDay < Infinity) {
                  this.noOfPostInDay[jj + 1] = daysEvents.length;
              }
              return (
                  <DayColumn
                      {...this.props}
                      ...
                      key={i + "-" + jj}
                      date={date}
                      events={daysEvents.slice(0, this.noOfPostsPerDay)}
                      backgroundEvents={daysBackgroundEvents}
                      dayLayoutAlgorithm={dayLayoutAlgorithm}
                      rotatedNode={this.state.rotatedNode}
                      updateRotatedNode={this.updateRotatedNode}
                      isSat={jj === 6}
                      isSun={jj === 0}
                      currentColumnInfo={this.state.currentColumnInfo}
                      updateCurrentColumnInfo={this.updateCurrentColumnInfo}
                      weekId={jj}
                      noOfPostInDay={this.noOfPostInDay}
                      noOfPostsPerDay={this.noOfPostsPerDay}
                      viewMoreCount={daysEvents?.slice(this.noOfPostsPerDay).length}
                      totalEvents={daysEvents}
                  />
              );
          })
      );
  ...
  render() {
      ...
      events.forEach((event) => {
          if (inRange(event, start, end, accessors, localizer)) {
              ...
              if (!allowDiffDateEvents && (
                  (event && this.props.isSocial && event.isAllDayEvent) ||  accessors.allDay(event) ||
      ...
      if (isSocial) {
          stableSortEvents(allDayEvents, accessors);
      } else {
      ...
      const onDragLeave = (e) => {
          ...
          const box = document.querySelector(".rbc-time-view");

          if (!relatedTarget || !box.contains(relatedTarget)) {
              //   const bottomCard = document.querySelector(`.bottom_card_${this.state.selectedEvent.id}`);
              //   bottomCard.style.visibility = "visible";

              const selectedDiv = document.getElementById(`card_${this.state.selectedEvent.id}`);
      ...
      const onDragEnter = () => {
          ...
          const selectedDiv = document.getElementById(`card_${this.state.selectedEvent.id}`);
          if (selectedDiv) selectedDiv.classList.add("dragged-box");
      };
      ...
      return (
          ...
          >
              <TimeGridHeader
                  ...
                  resizable={resizable}
                  appointmentHeader={this.props.appointmentHeader}
                  isSocial={this.props.isSocial}
                  isMonthView={this.props.isMonthView}
                  openDrawerForApple={this.props.openDrawerForApple}
                  toggleViewAllPostsDrawer={this.props.toggleViewAllPostsDrawer}
                  calendarSimmerStatus={this.props.calendarSimmerStatus}
                  showPlusIcon={this.props.showPlusIcon}
                  plusIconCallback={this.props.plusIconCallback}
                  customWeekAndMonthViewLoader={this.props.customWeekAndMonthViewLoader}
                  weekMaxRows={this.props.weekMaxRows}
              />
              {this.props.isSocial && (!isChrome && !isFireFox && isSafari) && <div id="dnd_reset_safari" onDragEnter={() => resetForSafari(this.state.currentColumnInfo.prevDay,this.state.selectedEvent,this.state.currentColumnInfo,this.updateCurrentColumnInfo)} style={{width:"100%",height:"35px",backgroundColor:"transparent",position:"absolute",top:"0",left:"0",zIndex:"-10"}} />}
              <div
                  ref={this.contentRef}
              ...
              >
                  <TimeGutter
                      ...
                      getters={getters}
                      showTimeSlots={this.props.showTimeSlots}
                  />
                  {this.renderEvents(
                      range,
                      rangeEvents,
...
TimeGrid.propTypes = {
    ...
    allowDiffDateEvents: PropTypes.bool,
