...
## 8. Settings contract checklist (forward standard)

- [ ] Every settings row click resolves to an explicit route target (`table`, `detail`, or placeholder state)
- [ ] L2 panel remains visible only on settings landing (`list`) and hides on drill-down views
- [ ] Breadcrumbs are clickable and always move state backward one level
- [ ] `TextTabsRow` mutates real view state (no no-op tab handlers)
- [ ] Top bar title reflects settings substate (`Settings`, `Business`, selected item name)
- [ ] Header actions use icon-only controls (`columns`, `more`, `filter`) where applicable
- [ ] New or edited settings UI components include matching Storybook coverage in `src/stories/App/Settings`
